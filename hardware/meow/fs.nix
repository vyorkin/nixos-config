{
  # Mount points are created automatically if they don’t already exist.
  # For device, it’s best to use the topology-independent device aliases in
  # /dev/disk/by-label and /dev/disk/by-uuid, as these don’t change if the
  # topology changes (e.g. if a disk is moved to another IDE controller).

  fileSystems."/" =
    { device = "/dev/disk/by-uuid/83ec9384-4631-4041-9d0e-1d800160e75d";
      fsType = "ext4";
    };

  fileSystems."/boot" =
    { device = "/dev/disk/by-uuid/5798-F9D4";
      fsType = "vfat";
    };

  swapDevices =
    [ { device = "/dev/disk/by-uuid/72ebd464-779a-4791-b1a8-3497900152f2"; }
    ];
}
